...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (sT->GetEnableTransmogInfo())
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|t幻化是如何工作的？", EQUIPMENT_SLOT_END + 9, 0);
...
#endif
        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|t移除所有幻化", EQUIPMENT_SLOT_END + 2, 0, "移除所有装备物品的幻化？", 0, false);
        ...
        switch (sender)
        {
            ...
            case EQUIPMENT_SLOT_END + 4: // Presets menu
            {
                ...
                if (sT->GetEnableSetInfo())
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|t套装是如何工作的？", EQUIPMENT_SLOT_END + 10, 0);
            ...
            case EQUIPMENT_SLOT_END + 6: // view preset
            {
                ...
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|t使用这个套装", EQUIPMENT_SLOT_END + 5, action, "使用此套装进行幻化会将幻化后的物品绑定到你身上，并使它们不可出售且不可交易。\n你想继续吗？\n\n" + sT->presetByName[player->GetGUID()][action], 0, false);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-LeaveItem-Opaque:30:30:-18:0|t删除套装", EQUIPMENT_SLOT_END + 7, action, "确定要删除" + sT->presetByName[player->GetGUID()][action] + "吗？", 0, false);
            ...
            case EQUIPMENT_SLOT_END + 8: // Save preset
            {
                ...
                if (canSave)
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|t保存套装", 0, 0, "输入套装名字", cost*sT->GetSetCostModifier() + sT->GetSetCopperCost(), true);
            ...
            if (sT->GetUseCollectionSystem())
            {
                ...
                if (sT->collectionCache.find(accountId) != sT->collectionCache.end())
                {
                    ...
                    if (pageNumber == 0)
                    {
                        if (hasSearchString)
                        {
                            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(30620, 30, 30, -18, 0) + "正在搜索: " + searchDisplayValue, slot + 1, 0, "搜索什么装备？", 0, true);
                        }
                        else
                        {
                            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(30620, 30, 30, -18, 0) + "搜索....", slot + 1, 0, "搜索什么装备？", 0, true);
                        }
                    }
                    else
                    {
                    ...
                    if (sT->GetAllowHiddenTransmog())
                    {
                        ...
                        else
                        {
                            ...
                            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "隐藏该部位", slot, UINT_MAX, "您正在将该部位的装备外观隐藏。\n确定要继续吗？\n\n" + lineEnd, 0, false);
                        }
                    }
                    ...
                    for (uint32 i = startValue; i <= endValue; i++)
                    {
                        ...
                        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sT->GetItemLink(newItem, session), slot, newItem->GetEntry(), "使用此套装进行幻化会将幻化后的物品绑定到你身上，并使它们不可出售且不可交易。\n你想继续吗？\n\n" + sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sT->GetItemLink(newItem, session) + lineEnd, price, false);
                    }
                }
                ...
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|t移除幻化", EQUIPMENT_SLOT_END + 3, slot, "确定要移除该部位的幻化？", 0, false);
            ...
            else
            {
                ...
                for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
                {
                    ...
                    if (sT->GetFakeEntry(oldItem->GetGUID()) == newItem->GetEntry())
                        continue;
                    ++limit;
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sT->GetItemLink(newItem, session), slot, newItem->GetGUID().GetCounter(), "使用此套装进行幻化会将幻化后的物品绑定到你身上，并使它们不可出售且不可交易。\n你想继续吗？\n\n" + sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sT->GetItemLink(newItem, session) + lineEnd, price, false);
                }
...
private:
    ...
    void AddToDatabase(Player* player, ItemTemplate const* itemTemplate)
    {
        ...
        {
            if (showChatMessage) {
                // ChatHandler(player->GetSession()).PSendSysMessage( R"(|c%s|Hitem:%u:0:0:0:0:0:0:0:0|h[%s]|h|r 已添加到您的外观收藏中。)", itemQuality.c_str(), itemId, itemName.c_str());
                ChatHandler(session).PSendSysMessage("%s 已添加到您的外观收藏。", sT->GetItemLink(itemId, session));
            }
